...
//Define application library path if we need to use it down the road
define('APPLICATION_LIBRARY',  realpath(__DIR__));
define('CACHE_DIR',  realpath(__DIR__) . '/tmp/');
define('LANG_FILE',  realpath(__DIR__) . '/lang/languages.json');
define('ROOT_PATH',  realpath(__DIR__) . '/../');
define('IMAGES_PATH',  realpath(__DIR__) . '/../pictures');
define('UPLOADS_PATH',  realpath(__DIR__) . '/../uploads');

define('APP_NAME', 'Geviner');

//Applicatiion related things
define('APP_URL', 'http://localhost/edukacija/edukacija/app/');
define('WEB_URL', 'http://localhost/edukacija/edukacija/');
...
//Lets autoload everything we want 
function __autoload($class) {
    if(is_file(APPLICATION_LIBRARY .'/classes/'.$class.".php")) {
        require_once APPLICATION_LIBRARY . '/classes/'.$class.'.php';
    }
    if(is_file(APPLICATION_LIBRARY . '/classes/models/'.$class.".php")) {
        require_once APPLICATION_LIBRARY . '/classes/models/'.$class.'.php';
    }
    if(is_file(APPLICATION_LIBRARY . '/classes/api/'.$class.".php")) {
        require_once APPLICATION_LIBRARY . '/classes/api/'.$class.'.php';
    }
}
...
function isMobileUA() {
    ...
    if (preg_match('/android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec' .
      '|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|' .
      'opera m(ob|in)i|palm( os)?|phone|p(ixi|re)' .
      '\/|plucker|pocket|psp|symbian|treo|up\.(browser|link)|vodafone' .
      '|wap|windows (ce|phone)|xda|xiino|ipad/i', $useragent)
      ||
      ...
      'ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|' .
      'wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|' .
      'gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|' .
      'hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|' .
      'iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|' .
      'ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|' .
      'kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|e\-|e\/|' .
      '\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|' .
      'mc(01|21|ca)|m\-cr|me(di|rc|ri)|mi(o8|oa|ts)|mmef|' .
  ...
  function isTabletUA() {
    ...
    $useragent = $_SERVER['HTTP_USER_AGENT'];
    foreach($tabletDevices as $regex) {
...
$applicationConfig  = Config::instance()->getConfig('application');

$active             = 'dashboard';
$subActive          = '';

$isLoggedUser = Website::isLoggedUser();
$remoteUser         = Administrator::getLoggedAdminInfo();
